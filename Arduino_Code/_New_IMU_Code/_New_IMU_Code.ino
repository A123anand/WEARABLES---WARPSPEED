...
// 2x MPU6050 + ESP32 + Quaternion Filter
//
// Wiring:
...
// ================================================================
// Self-Contained Madgwick Filter Struct
// No external library — full access to quaternion components
// ================================================================

struct MadgwickFilter {
float q0, q1, q2, q3; // quaternion components
float beta; // filter gain (higher = faster but noisier)
float sampleFreq;

void begin(float freq, float b = 0.033f) {
sampleFreq = freq;
beta = b;
// Start as identity quaternion (no rotation)
...
cal.ax = ax/N; cal.ay = ay/N; cal.az = (az/N) - 1.0f;
cal.gx = gx/N; cal.gy = gy/N; cal.gz = gz/N;
}

// ================================================================
// Angle Between Two Quaternion Orientations
// Uses relative quaternion — avoids gimbal lock from Euler subtraction
// ================================================================

float angleBetweenOrientations(MadgwickFilter &f1, MadgwickFilter &f2) {
// Dot product of two quaternions = cos(half-angle) of relative rotation
...
void setup() {
Serial.begin(115200);
Wire.begin(); // SDA=D21, SCL=D22 (ESP32 defaults)
