...
void setup() { 
    ...
    Serial.begin(9600); 
    SerialBT.begin("ESP32_IMU");  // Device name shown to laptop
...
void readSensorAngles(
    ...
    // --- existing angle code (optional to keep) ---
    int xAng = map(AcX, minVal, maxVal, -90, 90);
    int yAng = map(AcY, minVal, maxVal, -90, 90);
    int zAng = map(AcZ, minVal, maxVal, -90, 90);
...
void loop() { 
    ...
    // Read Spine IMU (0x68)
    readSensorAngles(MPU1_addr, "Spine (0x68)", spineAcX, spineAcY, spineAcZ);

    // Read Shoulder IMU (0x69)
    readSensorAngles(MPU2_addr, "Shoulder (0x69)", shoulderAcX, shoulderAcY, shoulderAcZ);
